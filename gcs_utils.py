...
def upload_image_to_gcs(local_path, filename, tipo):
    ...
    # Infer content type: prefer explicit mapping for .glb -> model/gltf-binary
        try:
        ...
        if content_type:
            ...
            def upload_image_to_gcs(local_path, filename, tipo):
                ...
                # Log em debug para não poluir stdout
                try:
                    logging.debug(f"[gcs_utils] Uploading file '{filename}' to bucket '{bucket.name}' (tipo={tipo})")
                except Exception:
                    pass
                ...
                try:
                    ...
                    # ignore and continue with default behavior
                    pass
                ...
                try:
                    ...
                    if isinstance(filename, str) and filename.endswith('/'):
                        ...
                        try:
                            ...
                            logging.warning(f"[gcs_utils] Error listing blobs with prefix '{prefix}' in bucket '{bucket.name}': {e}")
                            return False
                        if not blobs:
                            logging.debug(f"[gcs_utils] No objects found with prefix '{prefix}' in bucket '{bucket.name}'. Nothing to delete.")
                            return True
                        deleted = 0
                        for b in blobs:
                            try:
                                ...
                                logging.warning(f"[gcs_utils] Failed to delete blob '{b.name}' under prefix '{prefix}': {e}")
                                # continue deleting other blobs
                        logging.debug(f"[gcs_utils] Deleted {deleted} objects with prefix '{prefix}' from bucket '{bucket.name}'")
                        return True
